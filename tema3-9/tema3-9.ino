...
void loop() {
  ...
  if (digitalRead(con) == LOW) {
    ...
    if (digitalRead(condec) == LOW) {
    cuenta--; //restamos 1 cuando el segundo pulsador esta en low
      delay(1000);
  }
  if(cuenta > 9) { // si sumamos cuando hay 9, pasa a 0
    cuenta = 0;
}
  if(cuenta < 0) { // si restamos cuando hay un 0, pasará a 9
    cuenta = 9;
}
}
